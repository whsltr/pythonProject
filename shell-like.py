...
path = '/home/kun/Documents/mathematics/min_2017_case1/'
alpha_i = 87
f = open(path + str(alpha_i) + '_1_1.txt', 'w')
...
vtpar0 = [sqrt(0.04) * 4, sqrt(0.0005) * 4, sqrt(0.0002)]
# vtpar0=0.7/math.sqrt(2)
vtper0 = vtpar0
vtpar1 = 0.1 / sqrt(2)
# vtpar1=0.2/math.sqrt(2)
vtper1 = vtpar1
vtpar2 = sqrt(0.005) * sqrt(1836)
vtper2 = vtpar2
...
as_total = sum(a_s(theta, vtpar1, vtper1) for theta in range(-18, 200, 18))
while alpha <= alpha_max:
    ...
    while k >= k_min:
        ...
        for i in range(len(m0)):
            as0.append(sqrt(2) * k2 * vtper0[i] / m0[i])
        ...
        def A0(nn, vr, y_min, y_max):
            return ((y_max - y_min) / 300) * (fun_a0(nn, y_min, vr) + fun_a0(nn, y_max, vr) + 4 * sum(
                fun_a0(nn, y_min + (2 * j - 1) * ((y_max - y_min) / 100), vr) for j in range(1, 51))
                                              + 2 * sum(
                        fun_a0(nn, y_min + (2 * j) * ((y_max - y_min) / 100), vr) for j in range(1, 50)))


        def A1(nn, vr, y_min, y_max):
            return ((y_max - y_min) / 300) * (fun_a1(nn, y_min, vr) + fun_a1(nn, y_max, vr) + 4 * sum(
                fun_a1(nn, y_min + (2 * j - 1) * ((y_max - y_min) / 100), vr) for j in range(1, 51))
                                              + 2 * sum(
                        fun_a1(nn, y_min + (2 * j) * ((y_max - y_min) / 100), vr) for j in range(1, 50)))


        def B0(nn, vr, y_min, y_max):
            return ((y_max - y_min) / 300) * (fun_b0(nn, y_min, vr) + fun_b0(nn, y_max, vr) + 4 * sum(
                fun_b0(nn, y_min + (2 * j - 1) * ((y_max - y_min) / 100), vr) for j in range(1, 51))
                                              + 2 * sum(
                        fun_b0(nn, y_min + (2 * j) * ((y_max - y_min) / 100), vr) for j in range(1, 50)))


        def B1(nn, vr, y_min, y_max):
            return ((y_max - y_min) / 300) * (fun_b1(nn, y_min, vr) + fun_b1(nn, y_max, vr) + 4 * sum(
                fun_b1(nn, y_min + (2 * j - 1) * ((y_max - y_min) / 100), vr) for j in range(1, 51))
                                              + 2 * sum(
                        fun_b1(nn, y_min + (2 * j) * ((y_max - y_min) / 100), vr) for j in range(1, 50)))


        def C0(nn, vr, y_min, y_max):
            return ((y_max - y_min) / 300) * (fun_c0(nn, y_min, vr) + fun_c0(nn, y_max, vr) + 4 * sum(
                fun_c0(nn, y_min + (2 * j - 1) * ((y_max - y_min) / 100), vr) for j in range(1, 51))
                                              + 2 * sum(
                        fun_c0(nn, y_min + (2 * j) * ((y_max - y_min) / 100), vr) for j in range(1, 50)))


        def C1(nn, vr, y_min, y_max):
            return ((y_max - y_min) / 300) * (fun_c1(nn, y_min, vr) + fun_c1(nn, y_max, vr) + 4 * sum(
                fun_c1(nn, y_min + (2 * j - 1) * ((y_max - y_min) / 100), vr) for j in range(1, 51))
                                              + 2 * sum(
                        fun_c1(nn, y_min + (2 * j) * ((y_max - y_min) / 100), vr) for j in range(1, 50)))
        ...
        c0 = []
        c1 = []
        for theta in range(-18, 200, 18):
            vr1 = v_s * np.sin(theta / 360 * 2 * np.pi)
            bs1 = vr1 / (np.sqrt(2) * vtper1)

            ymin = max(0, bs1 - 5)
            ymax = bs1 + 5
            for n in range(nmin, nmax + 1):
                a0.append(A0(n, vr1, ymin, ymax))
                a1.append(A1(n, vr1, ymin, ymax))
                b0.append(B0(n, vr1, ymin, ymax))
                b1.append(B1(n, vr1, ymin, ymax))
                c0.append(C0(n, vr1, ymin, ymax))
                c1.append(C1(n, vr1, ymin, ymax))

        a0 = np.array(a0).reshape(13, nmax - nmin + 1)
        a1 = np.array(a1).reshape(13, nmax - nmin + 1)
        b0 = np.array(b0).reshape(13, nmax - nmin + 1)
        b1 = np.array(b1).reshape(13, nmax - nmin + 1)
        c0 = np.array(c0).reshape(13, nmax - nmin + 1)
        c1 = np.array(c1).reshape(13, nmax - nmin + 1)
        ...
        while delta < err < 1000:
            ...
            for omega in om:
                ...
                for ii in range(len(m0)):
                    ...
                    def eta0(omega_f, nn):
                        return (omega_f - k1 * vd0 - nn * m0[ii]) / (sqrt(2) * k1 * vtpar0[ii])


                    sc11 = sum(-2 * n * n / (as0[ii] * as0[ii]) * (
                            (vtper0[ii] ** 2) / (vtpar0[ii] ** 2) * A(n, as0[ii]) * DZ(eta0(omega, n))
                            + 2 * A(n, as0[ii]) * (1 - n * m0[ii] / (sqrt(2) * k1 * vtpar0[ii]) * Z(
                        eta0(omega, n)))) for n in range(nmin, nmax + 1))
                    sc12 = sum(
                        1j * 4 * n / as0[ii] * ((vtper0[ii] ** 2) / (vtpar0[ii] ** 2) * B(n, as0[ii]) * eta0(omega, n)
                                                + B(n, as0[ii]) * (
                                                        n * m0[ii] / (sqrt(2) * k1 * vtpar0[ii]))) * Z(
                            eta0(omega, n)) for n in range(nmin, nmax + 1))
                    sc13 = sum(-2 * n / as0[ii] * (vtper0[ii] / vtpar0[ii]) * (
                            (omega - n * m0[ii]) / (sqrt(2) * k1 * vtpar0[ii]) * A(n, as0[ii]) * DZ(eta0(omega, n))
                            - 2 * (vtpar0[ii] ** 2) / (vtper0[ii] ** 2) * n * m0[ii] / (sqrt(2) * k1 * vtpar0[ii]) * A(
                        n,
                        as0[
                            ii])
                            * (1 + (omega - n * m0[ii]) / (sqrt(2) * k1 * vtpar0[ii]) * Z(eta0(omega, n)))) for n in
                               range(nmin, nmax + 1))
                    sc21 = -sc12
                    sc22 = sum(-2 * ((vtper0[ii] ** 2) / (vtpar0[ii] ** 2) * C(n, as0[ii]) * DZ(eta0(omega, n))
                                     + 2 * C(n, as0[ii]) * (1 - n * m0[ii] / (sqrt(2) * k1 * vtpar0[ii]) * Z(
                                eta0(omega, n)))) for n in range(nmin, nmax + 1))
                    sc31 = sc13
                    sc23 = sum(1j * 2 * vtper0[ii] / vtpar0[ii] * (
                            (omega - n * m0[ii]) / (sqrt(2) * k1 * vtpar0[ii]) * B(n, as0[ii]) * DZ(eta0(omega, n))
                            - 2 * (vtpar0[ii] ** 2) / (vtper0[ii] ** 2) * n * m0[ii] / (sqrt(2) * k1 * vtpar0[ii]) * B(
                        n,
                        as0[
                            ii])
                            * (1 + (omega - n * m0[ii]) / (sqrt(2) * k1 * vtpar0[ii]) * Z(eta0(omega, n)))) for n in
                    ...
                    sc32 = -sc23
                    sc33 = sum(-2 * (
                            ((omega - n * m0[ii]) / (sqrt(2) * k1 * vtpar0[ii])) ** 2 * A(n, as0[ii]) * DZ(
                        eta0(omega, n))
                            - 2 * (vtpar0[ii] ** 2) / (vtper0[ii] ** 2) * (omega - n * m0[ii]) / (
                                    sqrt(2) * k1 * vtpar0[ii]) * n * m0[ii] / (
                                    sqrt(2) * k1 * vtpar0[ii]) * A(n, as0[ii])
                            * (1 + (omega - n * m0[ii]) / (sqrt(2) * k1 * vtpar0[ii]) * Z(eta0(omega, n)))) for n in
                               range(nmin, nmax + 1))
                    D11 += n0[ii] * m0[ii] * sc11
                    D12 += n0[ii] * m0[ii] * sc12
                    D13 += n0[ii] * m0[ii] * sc13
                    D21 += n0[ii] * m0[ii] * sc21
                    D22 += n0[ii] * m0[ii] * sc22
                    D23 += n0[ii] * m0[ii] * sc23
                    D31 += n0[ii] * m0[ii] * sc31
                    D32 += n0[ii] * m0[ii] * sc32
                    D33 += n0[ii] * m0[ii] * sc33
                ...
                D11 += omega * omega / (cva * cva) - k1 * k1 + (n2 * m22 * se11)
                D12 += (n2 * m22 * se12)
                D13 += k1 * k2 + (n2 * m22 * se13)
                D21 += (n2 * m22 * se21)
                D22 += omega * omega / (cva * cva) - kk + (n2 * m22 * se22)
                D23 += (n2 * m22 * se23)
                D31 += k1 * k2 + (n2 * m22 * se31)
                D32 += (n2 * m22 * se32)
                D33 += omega * omega / (cva * cva) - k2 * k2 + (n2 * m22 * se33)

                n_pick = 0
                i = 0
                for theta in range(-18, 200, 18):
                    vr1 = v_s * sin((theta / 360) * (2 * pi))
                    ...
                    # eta = as_total
                    n_pick = n_pick + eta
                    k_v = sqrt(2) * k1 * vtpar1


                    def eta1(omega_f, nn):
                        return (omega_f - k1 * vd1 - nn * m1) / k_v


                    sb11 = sum(
                        -2 * n * n / (as1 * as1 * As1) * (
                                (vtper1 ** 2) / (vtpar1 ** 2) * a0[i, n + nmax] * DZ(eta1(omega, n))
                                + 2 * a1[i, n + nmax] * (1 - n * m1 / k_v * Z(
                            eta1(omega, n)))) for n in range(nmin, nmax + 1))
                    sb12 = sum(
                        1j * 4 * n / (As1 * as1) * ((vtper1 ** 2) / (vtpar1 ** 2) * b0[i, n + nmax] * eta1(omega, n)
                                                    + b1[i, n + nmax] * (
                                                            n * m1 / k_v)) * Z(
                            eta1(omega, n)) for n in range(nmin, nmax + 1))
                    sb13 = sum(-2 * n / as1 / As1 * (vtper1 / vtpar1) * (
                            (omega - n * m1) / k_v * a0[i, n + nmax] * DZ(eta1(omega, n))
                            - 2 * (vtpar1 ** 2) / (vtper1 ** 2) * n * m1 / k_v * a1[i, n + nmax]
                            * (1 + (omega - n * m1) / k_v * Z(eta1(omega, n)))) for n in
                               range(nmin, nmax + 1))
                    sb21 = -sb12
                    sb22 = sum(-2 / As1 * ((vtper1 ** 2) / (vtpar1 ** 2) * c0[i, n + nmax] * DZ(eta1(omega, n))
                                           + 2 * c1[i, n + nmax] * (1 - n * m1 / k_v * Z(
                                eta1(omega, n)))) for n in range(nmin, nmax + 1))
                    sb23 = sum(1j * 2 / As1 * vtper1 / vtpar1 * (
                            (omega - n * m1) / k_v * b0[i, n + nmax] * DZ(eta1(omega, n))
                            - 2 * (vtpar1 ** 2) / (vtper1 ** 2) * n * m1 / k_v * b1[i, n + nmax]
                            * (1 + (omega - n * m1) / k_v * Z(eta1(omega, n)))) for n in
                               range(nmin, nmax + 1))
                    sb31 = sb13
                    sb32 = -sb23
                    sb33 = sum(-2 / As1 * (
                        ...
                        eta1(omega, n))
                            - 2 * (vtpar1 ** 2) / (vtper1 ** 2) * (omega - n * m1) / k_v * n * m1 / k_v * a1[
                                i, n + nmax]
                            * (1 + (omega - n * m1) / k_v * Z(eta1(omega, n)))) for n in
                               range(nmin, nmax + 1))
                    i = i + 1
            ...
            err = abs((omega3 - omega2) / omega3) if abs(c) < 10 ** -2 else 1
            omega0 = omega1
            omega1 = omega2
            omega2 = omega3
            om = [omega0, omega1, omega2]
