...
for t in range(0, 8001, 500):
    ...
    x = []
    y = []
    for rank in range(0, 256, 10):
        index = 0
        i, vx1, vy1, vz1, x1, y1 = data.read_phase(path, rank, t)
        i = np.array(i)
        vx += vx1
        vy += vy1
        vz += vz1
...
# background proton
    ...
    h_b = plt.hist2d(v_para, v_per, bins=200, range=[[-12, 12], [0, 15]])
    ...
    col = ax.pcolormesh(X_b, Y_b, np.log10((value_b / (index * yedges_b[1:]))).T, shading='gouraud', cmap=palette,
                        vmax=0)
    ...
    # ax.set_xlim(-12, 12)
    ax.axis('scaled')
    ax.set_title(r'$t\Omega_i=$' + str(t * 0.01), fontsize=16)
    ax.set_xlabel('$v_{||}/v_A$', font1)
    ax.set_ylabel('$v_{\perp}/v_A$', font1)
    ax.tick_params(labelsize=12)
    plt.colorbar(col)
    plt.savefig(path + 'density_oxy' + str(t) + '.png')
